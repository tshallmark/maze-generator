...
Maze::Maze(sf::RenderWindow* w,int cols, int rows)
{
    ...
    renderLoad( sf::Vector2i(0,start) );
    renderLoad( sf::Vector2i(rowSize-1,end) );

    refresh();
}

void Maze::randomImperfect(int walls)
{
    for (size_t i = 0; i < walls; i++)
    {
        int x =  2 * (rand() % (colSize / 2));
        int y =  1 + 2 * (rand() % (rowSize / 2));
        grid[y][x] = 'o';
        renderLoad(sf::Vector2i(x,y));
    }
}
...
void Maze::draw(bool all)
{
    ...
    while(!renderQ.empty())
    {
        ...
        for(size_t i = 0; i < speed; i++)
        {
            ...
            if(!renderWalls)
            {
                posX = 0 + (((float)temp.x-1) * squareSize.x);
                posY = 0 + ((float)temp.y * squareSize.y);
                drawSquare(c, posX, posY, curr.val);
                posX = 0 + ((float)temp.x * squareSize.x);
                posY = 0 + (((float)temp.y-1) * squareSize.y);
                drawSquare(c, posX, posY, curr.val);
                posX = 0 + (((float)temp.x-1) * squareSize.x);
                posY = 0 + (((float)temp.y-1) * squareSize.y);
                drawSquare(c, posX, posY, curr.val);
...
sf::Color Maze::getColor(char c, int val)
{
    ...
    case 'f':
    case 'g':
        ratio = (float)val / maxDistance;
        step = 255 * ratio;
        fade = sf::Color(step, 0, 255-step);
        return fade;
        break;
    case 'p':
...
void Maze::renderLoad(sf::Vector2i pos, int val)
{
    ...
    renderQ.push(renderNode(pos, c, val));
}
 ...
 */
bool Maze::validMove(sf::Vector2i loc, char exception)
{
    int row = loc.y;
    int col = loc.x;
    //return true if
...
void DivMaze::randHole(sf::Vector2i a, sf::Vector2i b)
{
    ...
    {
        grid[a.y][a.x] = 'o';
        renderLoad(sf::Vector2i(a.x,a.y));
    }
